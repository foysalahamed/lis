...
public class FeeStructureSetup extends JFrame {
	...
	JComboBox comboBoxFeeOccuranceDB;
	JComboBox comboBoxFeeCategoriesDB;
	...
	public FeeStructureSetup() {
		...
		comboBoxFeeOccurance.setModel(new DefaultComboBoxModel(new String[] {"Select", "Admition", "Exam", "Sports", "Picnic", "Tuition Fee", "Others"}));
		comboBoxFeeOccurance.setBounds(129, 61, 110, 20);
		...
		textFieldFeeCategories.setBounds(129, 83, 110, 20);
		...
		btnClear.setBounds(100, 181, 60, 23);
		panel.add(btnClear);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					PreparedStatement prStatement= connection.prepareStatement(query);
					prStatement.setString(1,(String)comboBoxFeeOccuranceDB.getSelectedItem() );
					prStatement.setString(2,(String)comboBoxFeeCategoriesDB.getSelectedItem() );
					prStatement.setString(3,(String)comboBoxFeeOccuranceDB.getSelectedItem() );
					prStatement.setString(4,textFieldFeeAmount.getText() );
					if(chckbxNursery.isSelected()){
						prStatement.setString(5,"Nursery" );

					}
					if(chckbxPlay.isSelected()){
						prStatement.setString(5,"Play" );

					}

					if(chckbxYearOne.isSelected()){
						prStatement.setString(5,"One" );

					}
					if(chckbxYearTwo.isSelected()){
						prStatement.setString(5,"Two" );

					}
					if(chckbxYearThree.isSelected()){
						prStatement.setString(5,"Three" );

					}
					if(chckbxYearFour.isSelected()){
						prStatement.setString(5,"Four" );

					}
					if(chckbxYearFive.isSelected()){
						prStatement.setString(5,"Five" );

					}
					if(chckbxYearSix.isSelected()){
						prStatement.setString(5,"Six" );

					}
					
					
					
					//prStatement.setString(5,(String)c.getSelectedItem() );
		...
		panel_1.add(lblFee);
		 ...
		 comboBoxFeeOccuranceDB = new JComboBox();
		 comboBoxFeeOccuranceDB.addActionListener(new ActionListener() {
		 	public void actionPerformed(ActionEvent arg0) {
		 		comboBoxFeeOccuranceDB.addItem("garbeg");
		 		
		 		String str=comboBoxFeeOccuranceDB.getSelectedItem().toString();
		 		comboBoxFeeOccuranceDB.removeItem("garbeg");
		 		comboBoxFeeCategoriesDB.removeAllItems();
				...
				try {
					...
					while(result.next()){
						comboBoxFeeCategoriesDB.addItem(result.getString("FEE_ID"));
		...
		comboBoxFeeOccuranceDB.setBounds(128, 66, 110, 20);
		panel_1.add(comboBoxFeeOccuranceDB);
		
		 comboBoxFeeCategoriesDB = new JComboBox();
		comboBoxFeeCategoriesDB.setBounds(128, 89, 110, 20);
		panel_1.add(comboBoxFeeCategoriesDB);
		...
		btnEdit_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String feeid=comboBoxFeeCategoriesDB.getSelectedItem().toString();
				double fee=Double.parseDouble(textFieldFeeAmount.getText());
				String query="update lis_fee_setup set amount='"+fee+"' where fee_id='"+feeid+"' and applicable_for='"+yr+"'";
				try {
	...
	protected void clear_fee_setup() {
		// TODO Auto-generated method stub
		comboBoxFeeOccuranceDB.setSelectedIndex(0);
	...
	private void comboBoxFeeOccuranceDBSetup() {
		// TODO Auto-generated method stub
		comboBoxFeeOccuranceDB.removeAllItems();
		...
		try {
			...
			ResultSet result=prstatement.executeQuery();
			while(result.next()){
				comboBoxFeeOccuranceDB.addItem(result.getString("FEE_NAME"));
				///////////////////////////////////////System.out.print(result.getString("FEE_OCCURANCES"));
				//////////////////////////////////////////////comboBoxFeeOccuranceDB.size();
			}	
